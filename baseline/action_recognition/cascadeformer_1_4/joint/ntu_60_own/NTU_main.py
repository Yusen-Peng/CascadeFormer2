...
def main():
    ...
    if pretrain: 
        ...
        torch.save(model.state_dict(), "action_checkpoints/NTU_BIO/NTU_pretrained.pt")
    ...
    three_d = True
    t1 = load_T1(
        model_path="action_checkpoints/NTU_BIO/NTU_pretrained.pt",
        num_joints=NUM_JOINTS_NTU,
        three_d=three_d,
        d_model=hidden_size,
        nhead=n_heads,
    ...
    torch.save(trained_T2.state_dict(), "action_checkpoints/NTU_BIO/NTU_finetuned_T2.pt")
    torch.save(train_cross_attn.state_dict(), "action_checkpoints/NTU_BIO/NTU_finetuned_cross_attn.pt")
    torch.save(train_head.state_dict(), "action_checkpoints/NTU_BIO/NTU_finetuned_head.pt")

    if any(param.requires_grad for param in t1.parameters()):
        torch.save(t1.state_dict(), "action_checkpoints/NTU_BIO/NTU_finetuned_T1.pt")
