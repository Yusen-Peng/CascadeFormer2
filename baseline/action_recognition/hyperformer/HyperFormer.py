...
def _no_grad_trunc_normal_(tensor, mean, std, a, b):
    ...
    with torch.no_grad():
        ...
        lm = norm_cdf((a - mean) / std)
        u = norm_cdf((b - mean) / std)

        # Uniformly fill tensor with values from [l, u], then translate to
        # [2l-1, 2u-1].
        tensor.uniform_(2 * lm - 1, 2 * u - 1)
...
def get_spatial_graph(num_node, self_link, inward, outward):
    IU = edge2mat(self_link, num_node)
    In = normalize_digraph(edge2mat(inward, num_node))
    Out = normalize_digraph(edge2mat(outward, num_node))
    A = np.stack((IU, In, Out))
    return A
...
def k_adjacency(A, k, with_self=False, self_factor=1):
    ...
    IU = np.eye(len(A), dtype=A.dtype)
    if k == 0:
        return IU
    Ak = np.minimum(np.linalg.matrix_power(A + IU, k), 1) \
       - np.minimum(np.linalg.matrix_power(A + IU, k - 1), 1)
    if with_self:
        Ak += (self_factor * IU)
    return Ak
